...
#' }
lawn_count <- function(polygons, points, in_field, out_field = 'count', lint = FALSE) {
  lawnlint(list(polygons, points), lint)
  ct$eval(sprintf("var fc = turf.collect(%s, %s, '%s', 'values');", convert(polygons), convert(points), in_field))
